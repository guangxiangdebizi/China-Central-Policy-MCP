...
export const getPolicyFulltext = {
  ...
  async run(args: { url: string; policy_id?: string }) {
    try {
...
function parseGovCnPolicyHtml(html: string, url: string) {
...
function inferIssuerFromText(text: string): string {
  const patterns = [
    /(国务院)/,
    /(全国人民代表大会)/,
    /(中国人民银行)/,
