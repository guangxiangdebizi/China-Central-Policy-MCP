...
async function searchPolicyAPI(keyword, limit, startdate, enddate) {
    const items = [];
    const requestedUrls = [];
    const page = 1;
    ...
    try {
        ...
        if (startdate)
            params.append('mintime', startdate);
        if (enddate)
            params.append('maxtime', enddate);
        ...
        if (data && Array.isArray(data.results)) {
            for (const result of data.results) {
                ...
                if (!policy.title || !policy.url || policy.title.length < 3)
                    continue;
                items.push(policy);
            }
        }
        ...
        const catMapCandidate = (data && (data.catMap || data?.searchVO?.catMap || data?.data?.catMap)) || undefined;
        if (catMapCandidate && typeof catMapCandidate === "object") {
            const catEntries = Object.entries(catMapCandidate).filter(([, v]) => v && Array.isArray(v.listVO));
...
async function searchPolicyFallback(keyword, limit) {
    try {
...
function formatDateFromEpoch(epochMaybe) {
...
async function fetchFromGovCnLatest(limit, startdate, enddate) {
    const items = [];
    const seen = new Set();
    let startDate = null;
    let endDate = null;
...
async function fetchGovCnPage(pageNum) {
